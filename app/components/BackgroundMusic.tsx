...
const PLAYLISTS_CACHE_STORAGE_KEY = "cd_music_playlists_v1";
...
function buildShuffledOrder(length: number) {
...
function readCachedPlaylists() {
  if (typeof window === "undefined") return [] as Playlist[];

  try {
    const raw = window.localStorage.getItem(PLAYLISTS_CACHE_STORAGE_KEY);
    if (!raw) return [] as Playlist[];
    ...
    if (typeof parsed.expiresAt !== "number" || parsed.expiresAt < Date.now()) {
      window.localStorage.removeItem(PLAYLISTS_CACHE_STORAGE_KEY);
      return [] as Playlist[];
    }

    return isPlaylistArray(parsed.playlists) ? parsed.playlists : ([] as Playlist[]);
  } catch {
    return [] as Playlist[];
  }
}

function writeCachedPlaylists(playlists: Playlist[]) {
  if (typeof window === "undefined") return;

  try {
    window.localStorage.setItem(
      PLAYLISTS_CACHE_STORAGE_KEY,
      JSON.stringify({
...
export default function BackgroundMusic() {
  ...
  useEffect(() => {
    ...
    async function loadPlaylists() {
      try {
        ...
        writeCachedPlaylists(nextPlaylists);
        setLoadError(null);
      } catch (error) {
        if (cancelled) return;
        if (!hasCachedPlaylists) {
  ...
  useEffect(() => {
    ...
    setTrackOrder(buildShuffledOrder(selectedPlaylist.tracks.length));
    setTrackPosition(0);
  }, [selectedPlaylist]);

  useEffect(() => {
    if (!selectedPlaylist || selectedPlaylist.tracks.length === 0) return;
    if (trackOrder.length === selectedPlaylist.tracks.length && trackPosition < trackOrder.length) {
      return;
    }

    setTrackOrder(buildShuffledOrder(selectedPlaylist.tracks.length));
    setTrackPosition(0);
  }, [selectedPlaylist, trackOrder, trackPosition]);

  const currentTrackIndex =
    selectedPlaylist && trackOrder.length > 0
      ? trackOrder[Math.min(trackPosition, trackOrder.length - 1)] ?? 0
      : 0;
  ...
  }, [currentTrackIndex, isPlaying, selectedPlaylist]);
  ...
  function advanceTrack() {
    ...
    if (trackPosition >= trackOrder.length - 1) {
      setTrackOrder(buildShuffledOrder(selectedPlaylist.tracks.length));
      setTrackPosition(0);
      return;
    }
  ...
  return (
    <div className="fixed bottom-5 left-5 z-[70] flex flex-col items-start gap-3">
      <audio ref={audioRef} onEnded={handleTrackEnd} preload="metadata" hidden />
      ...
      >
        <span className="relative inline-flex h-6 w-8 items-center justify-center rounded-full">
          ...
          >
            {[18, 32, 22, 42, 50, 34, 18, 30, 20].map((height, index) => (
              <span
                key={`${height}-${index}`}
