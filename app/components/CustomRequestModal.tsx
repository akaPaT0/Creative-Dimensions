...
export default function CustomRequestModal({
  ...
  const waText = () => {
    const line = `Custom request: ${productName || ""}${productUrl ? ` — ${productUrl}` : ""}`;
    const extra = details.trim() ? `\nDetails: ${details.trim()}` : "";
    return `${line}${extra}`.trim();
  };
    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-[80] flex items-end sm:items-center justify-center">
          ...
          <div className="relative z-[81] w-full sm:max-w-lg rounded-t-3xl sm:rounded-3xl border border-white/10 bg-[#0D0D0D]/70 backdrop-blur-xl backdrop-saturate-150 p-5 sm:p-6 text-white">
