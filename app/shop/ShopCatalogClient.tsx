    ...
    >
      <div className="flex items-center justify-between gap-3 text-left">
        <div>
          ...
          <p className="text-xs text-white/50 max-sm:hidden mt-0.5">
        ...
        <button
          type="button"
          onClick={resetAll}
          className="hidden rounded-xl border border-white/15 bg-white/5 px-3.5 py-1.5 text-xs text-white transition hover:bg-white/10 sm:inline-flex"
        >
          Reset
        </button>
      </div>
      ...
      <div className="mt-4 flex items-center justify-between text-xs text-white/50">
        ...
        <div className="hidden sm:block">
          Search hint: try <span className="text-white/80">key</span> or <span className="text-white/80">bmw</span>
        </div>
      </div>
      ...
      >
        {filtered.map((p) => (
          <Link
            ...
            className="group rounded-xl border border-white/10 bg-black/20 p-3 sm:p-4 hover:bg-black/40 hover:border-white/20 transition flex flex-col justify-between"
          >
            <div>
              ...
              <div className="mt-3 flex items-start justify-between gap-2">
                <div className="min-w-0 font-semibold text-white text-sm line-clamp-2 leading-snug group-hover:text-[#FF8B64] transition">
                  {getTitle(p)}
                </div>
                <div className="shrink-0 text-[#FF8B64] font-semibold text-xs pt-0.5">
                  {getPriceLabel(p)}
                </div>
              </div>

              <div className="mt-1 hidden sm:block text-xs text-white/50 line-clamp-2">
                {getDesc(p)}
              </div>
            </div>

            <div className="mt-3 pt-2.5 border-t border-white/10 text-[11px] text-white/40 flex justify-between">
      ...
      {filtered.length === 0 && (
        <div className="mt-8 text-center text-white/50 text-sm py-8">
          No matches found. Try a different search term or reset filters.
        </div>
      )}
