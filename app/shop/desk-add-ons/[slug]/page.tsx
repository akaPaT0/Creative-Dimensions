...
async function getProduct(slug: string) {
  ...
  return products.find(
    (x) => x.category === "desk-add-ons" && slugify(String(x.slug)) === slug
  );
}
...
export async function generateMetadata({
  ...
  return buildProductMetadata(p, "desk-add-ons");
}
...
export default async function DeskAddOnsSlugPage({
  ...
  const p = products.find(
    (x) => x.category === "desk-add-ons" && slugify(String(x.slug)) === slugify(rawSlug)
  );
  ...
  const productUrl = `${SITE_URL}/shop/desk-add-ons/${encodeURIComponent(cleanSlug)}`;
  const { productSchema, breadcrumbSchema } = buildProductJsonLd(p, "desk-add-ons");
