...
export default async function Page() {
  ...
  const hasAnySubCats = accessories.some((p) => Boolean(p.subCategory));
  const list = hasAnySubCats
    ? accessories.filter((p) => Boolean(p.subCategory))
    : accessories;
  ...
  return (
    <div className="relative min-h-screen">
      ...
      <main className="relative z-10 mx-auto max-w-7xl px-6 pt-28 pb-16">
        ...
        <div className="mt-8">
          <h1 className="text-4xl font-semibold text-white">Accessories</h1>
          <p className="mt-2 text-white/70">Browse our accessories.</p>
        </div>
        ...
        <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {list.map((p) => (
            <Link
              key={p.id}
              href={`/shop/accessories/${encodeURIComponent(p.slug)}`}
