...
function getCardImage(p: Product) {
  if (Array.isArray(p.images) && p.images.length > 0) {
    if (p.images[0] === "placeholder") return "/products/placeholder.jpg";
    return p.images[0];
  }
...
function getPriceLabel(p: Product) {
  if (typeof p.priceUSD === "number") return `${p.priceUSD} USD`;
  return "DM for price";
}
...
function getStableKey(p: Product) {
  return `${p.slug ?? "no-slug"}-${p.SKU ?? "no-sku"}`;
}

function VanessaSearchBar({
  products,
  value,
  onValueChange,
  onPickCategory,
...
export default function VanessaCatalogClient({
  products,
}: {
  products: Product[];
}) {
  ...
  const filtered = useMemo(() => {
    ...
    if (q) {
      list = list.filter((p) => {
        const hay = `${p.SKU || ""} ${getTitle(p)} ${p.slug} ${p.category} ${
          p.subCategory || ""
    ...
    if (sort === "price-asc") {
      list.sort(
        (a, b) => (Number(a.priceUSD ?? 0) || 0) - (Number(b.priceUSD ?? 0) || 0)
      );
    } else if (sort === "price-desc") {
      list.sort(
        (a, b) => (Number(b.priceUSD ?? 0) || 0) - (Number(a.priceUSD ?? 0) || 0)
      );
    }
